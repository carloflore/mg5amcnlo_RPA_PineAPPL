      SUBROUTINE DLUM_2(LUM)
C     ****************************************************            
C         
C     Generated by MadGraph5_aMC@NLO v. 3.5.7.RPA.1.0.0, 2025-01-22
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     RETURNS PARTON LUMINOSITIES FOR MADFKS                          
C        
C     
C     Process: g u~ > t t~ u~ [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: g c~ > t t~ c~ [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: g d~ > t t~ d~ [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: g s~ > t t~ s~ [ all = QCD QED ] QCD^2=6 QED^2=0
C     
C     ****************************************************            
C         
      IMPLICIT NONE
C     
C     CONSTANTS                                                       
C         
C     
      INCLUDE 'genps.inc'
      INCLUDE 'nexternal.inc'
      DOUBLE PRECISION       CONV
      PARAMETER (CONV=389379660D0)  !CONV TO PICOBARNS             
C     
C     ARGUMENTS                                                       
C         
C     
      DOUBLE PRECISION LUM
C     
C     LOCAL VARIABLES                                                 
C         
C     
      INTEGER I, ICROSS,LP
      DOUBLE PRECISION G1
      DOUBLE PRECISION CX2,SX2,UX2,DX2
C     
C     EXTERNAL FUNCTIONS                                              
C         
C     
      DOUBLE PRECISION PDG2PDF
C     
C     GLOBAL VARIABLES                                                
C         
C     
      INTEGER              IPROC
      DOUBLE PRECISION PD(0:MAXPROC)
      COMMON /SUBPROC/ PD, IPROC

      INTEGER             IPROS, IPROSS
      DOUBLE PRECISION PD1(0:MAXPROC), PD2(0:MAXPROC)
      COMMON /PDFVALUES/ PD1, PD2

      INCLUDE 'coupl.inc'
      INCLUDE 'run.inc'
      INTEGER IMIRROR
      COMMON/CMIRROR/IMIRROR
C     
C     STUFF FOR DRESSED EE COLLISIONS
C     
      INCLUDE 'eepdf.inc'
      DOUBLE PRECISION EE_COMP_PROD
      DOUBLE PRECISION DUMMY_COMPONENTS(N_EE)
      DOUBLE PRECISION G1_COMPONENTS(N_EE)
      DOUBLE PRECISION CX2_COMPONENTS(N_EE),SX2_COMPONENTS(N_EE)
     $ ,UX2_COMPONENTS(N_EE),DX2_COMPONENTS(N_EE)

      INTEGER I_EE
      INCLUDE '../../Source/PDF/pdf.inc'
C     
C     DATA                                                            
C         
C     
      DATA G1/1*1D0/
      DATA CX2,SX2,UX2,DX2/4*1D0/
      DATA ICROSS/1/
C     ----------                                                      
C         
C     BEGIN CODE                                                      
C         
C     ----------                                                      
C         
      LUM = 0D0
      IF (ABS(LPP(1)) .GE. 1) THEN
        G1=PDG2PDF(LPP(1),0,1,XBK(1),DSQRT(Q2FACT(1)))
        IF ((ABS(LPP(1)).EQ.4.OR.ABS(LPP(1)).EQ.3)
     $   .AND.PDLABEL.NE.'none') G1_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      IF (ABS(LPP(2)) .GE. 1) THEN
        CX2=PDG2PDF(LPP(2),-4,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') CX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
        SX2=PDG2PDF(LPP(2),-3,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') SX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
        UX2=PDG2PDF(LPP(2),-2,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') UX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
        DX2=PDG2PDF(LPP(2),-1,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') DX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u~ > t t~ u~
      PD(IPROC) = G1*UX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(G1_COMPONENTS,UX2_COMPONENTS)
      IPROC=IPROC+1  ! g c~ > t t~ c~
      PD(IPROC) = G1*CX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(G1_COMPONENTS,CX2_COMPONENTS)
      IPROC=IPROC+1  ! g d~ > t t~ d~
      PD(IPROC) = G1*DX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(G1_COMPONENTS,DX2_COMPONENTS)
      IPROC=IPROC+1  ! g s~ > t t~ s~
      PD(IPROC) = G1*SX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(G1_COMPONENTS,SX2_COMPONENTS)

      PD2(0) = 0D0
      IPROS = 0

      IPROS=IPROS+1  ! g u~ > t t~ u~
      PD2(IPROS) = UX2

      IPROS=IPROS+1  ! g c~ > t t~ c~
      PD2(IPROS) = CX2

      IPROS=IPROS+1  ! g d~ > t t~ d~
      PD2(IPROS) = DX2

      IPROS=IPROS+1  ! g s~ > t t~ s~
      PD2(IPROS) = SX2

      PD1(0) = 0D0
      IPROSS = 0

      IPROSS=IPROSS+1  ! g u~ > t t~ u~
      PD1(IPROSS) = G1

      IPROSS=IPROSS+1  ! g c~ > t t~ c~
      PD1(IPROSS) = G1

      IPROSS=IPROSS+1  ! g d~ > t t~ d~
      PD1(IPROSS) = G1

      IPROSS=IPROSS+1  ! g s~ > t t~ s~
      PD1(IPROSS) = G1
      DO I=1,IPROC
        IF (NINCOMING.EQ.2) THEN
          LUM = LUM + PD(I) * CONV
        ELSE
          LUM = LUM + PD(I)
        ENDIF
      ENDDO
      RETURN
      END

