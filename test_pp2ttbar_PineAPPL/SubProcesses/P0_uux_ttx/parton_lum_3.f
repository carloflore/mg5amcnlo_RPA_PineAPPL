...
C        
C     
C     Process: u g > t t~ u [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: c g > t t~ c [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: d g > t t~ d [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: s g > t t~ s [ all = QCD QED ] QCD^2=6 QED^2=0
C     
C     ****************************************************            
C         
      IMPLICIT NONE
...
C         
     ...
     $ =EE_COMP_PROD(C1_COMPONENTS,G2_COMPONENTS)
      ...
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(D1_COMPONENTS,G2_COMPONENTS)
      IPROC=IPROC+1  ! s g > t t~ s
      PD(IPROC) = S1*G2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(S1_COMPONENTS,G2_COMPONENTS)

      PD2(0) = 0D0
      IPROS = 0

      IPROS=IPROS+1  ! u g > t t~ u
      PD2(IPROS) = G2

      IPROS=IPROS+1  ! c g > t t~ c
      PD2(IPROS) = G2

      IPROS=IPROS+1  ! d g > t t~ d
      PD2(IPROS) = G2

      IPROS=IPROS+1  ! s g > t t~ s
      PD2(IPROS) = G2

      PD1(0) = 0D0
      IPROSS = 0

      IPROSS=IPROSS+1  ! u g > t t~ u
      PD1(IPROSS) = U1

      IPROSS=IPROSS+1  ! c g > t t~ c
      PD1(IPROSS) = C1

      IPROSS=IPROSS+1  ! d g > t t~ d
      PD1(IPROSS) = D1

      IPROSS=IPROSS+1  ! s g > t t~ s
      PD1(IPROSS) = S1
      DO I=1,IPROC
