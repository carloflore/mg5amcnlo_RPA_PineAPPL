...
C     ****************************************************            
C         
C     Generated by MadGraph5_aMC@NLO v. 3.5.7.RPA.1.0.0, 2025-01-22
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     RETURNS PARTON LUMINOSITIES FOR MADFKS                          
C        
C     
C     Process: u u~ > t t~ g [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: c c~ > t t~ g [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: d d~ > t t~ g [ all = QCD QED ] QCD^2=6 QED^2=0
C     Process: s s~ > t t~ g [ all = QCD QED ] QCD^2=6 QED^2=0
C     
C     ****************************************************            
C         
      IMPLICIT NONE
...
C     
      ...
      DOUBLE PRECISION D1_COMPONENTS(N_EE),U1_COMPONENTS(N_EE)
     $ ,S1_COMPONENTS(N_EE),C1_COMPONENTS(N_EE)
      DOUBLE PRECISION CX2_COMPONENTS(N_EE),SX2_COMPONENTS(N_EE)
     $ ,UX2_COMPONENTS(N_EE),DX2_COMPONENTS(N_EE)
...
C         
     ...
     $ =EE_COMP_PROD(S1_COMPONENTS,SX2_COMPONENTS)
      ...
      IPROS=IPROS+1  ! u u~ > t t~ g
      PD2(IPROS) = UX2

      IPROS=IPROS+1  ! c c~ > t t~ g
      PD2(IPROS) = CX2

      IPROS=IPROS+1  ! d d~ > t t~ g
      PD2(IPROS) = DX2

      IPROS=IPROS+1  ! s s~ > t t~ g
      PD2(IPROS) = SX2

      PD1(0) = 0D0
      IPROSS = 0

      IPROSS=IPROSS+1  ! u u~ > t t~ g
      PD1(IPROSS) = U1

      IPROSS=IPROSS+1  ! c c~ > t t~ g
      PD1(IPROSS) = C1

      IPROSS=IPROSS+1  ! d d~ > t t~ g
      PD1(IPROSS) = D1

      IPROSS=IPROSS+1  ! s s~ > t t~ g
      PD1(IPROSS) = S1
      DO I=1,IPROC
